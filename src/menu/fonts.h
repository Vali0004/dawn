...
namespace cs::renderer::fonts {
	class font {
	public:
		font(const stdfs::path& path, const f32& size) : m_font(nullptr), m_path(path), m_type(0), m_size(size) {
			m_name = path.filename().string();
		}
		font(const stdfs::path& path, const f32& size, ImFontConfig cfg) : m_config(cfg), m_font(nullptr), m_path(path), m_type(0), m_size(size) {
			m_name = path.filename().string();
		}
		font(const std::string& name, const f32& size, u8* memory, u64 memory_size) : m_font(nullptr), m_name(name), m_type(1), m_size(size) {
			m_mem.resize(memory_size);
			memcpy(m_mem.data(), memory, m_mem.size());
		}
		font(const std::string& name, const f32& size, const u8* memory, u64 memory_size) : m_font(nullptr), m_name(name), m_type(1), m_size(size) {
			m_mem.resize(memory_size);
			memcpy(m_mem.data(), memory, m_mem.size());
		}
		font(const std::string& name, const f32& size, u8* memory, u64 memory_size, ImFontConfig cfg) : m_config(cfg), m_font(nullptr), m_name(name), m_type(1), m_size(size) {
			m_mem.resize(memory_size);
			memcpy(m_mem.data(), memory, m_mem.size());
		}
		font(const std::string& name, const f32& size, const u8* memory, u64 memory_size, ImFontConfig cfg) : m_config(cfg), m_font(nullptr), m_name(name), m_type(1), m_size(size) {
			m_mem.resize(memory_size);
			memcpy(m_mem.data(), memory, m_mem.size());
		}
		explicit font(const f32& size) : m_font(nullptr), m_name("default"), m_type(2), m_size(size) {}
		font(const f32& size, ImFontConfig cfg) : m_config(cfg), m_font(nullptr), m_name("default"), m_type(2), m_size(size) {}
