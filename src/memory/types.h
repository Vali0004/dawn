...
namespace cs::pointers::types
{
	...
	using sysEpicInitClassT = fptr<bool(void* _This, const char* epicProductName, char* epicProductVersion, const char* epicProductId, const char* epicSandboxId, const char* epicClientId, const char* epicClientSecret, const char* epicDeploymentId)>;
	using entitlementManagerInitT = fptr<void(u32 v)>;
	using fiAssetManagerFullPathT = fptr<void(void* _This, char* dest, int maxLen, const char* base, const char* ext, int pathIndex)>;
	using grcEffectInitT = fptr<bool(rage::grcEffect* effect, const char* filename)>;
	using grcEffectCreateT = fptr<rage::grcEffect* (const char* filename)>;
	using CommandShouldWarnOfSimpleModCheckT = fptr<bool()>;
	using grcBeginT = fptr<void(rage::grcDrawMode dm, int count)>;
	using grcVertexT = fptr<void(float x, float y, float z, float nx, float ny, float nz, rage::Color32 c, float s, float t)>;
	using grcEndT = fptr<void()>;
	using grcStateBlockCreateRasterizerStateT = fptr<rage::grcStateBlock::grcRasterizerStateHandle(rage::grcStateBlock::grcRasterizerStateDesc const& state, char const*)>;
